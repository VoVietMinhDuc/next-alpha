...
### Đã chọn: Hướng D — store là nguồn sự thật
Lý do: **đề bài chốt cứng** *"uploads only what changed"* + log added/updated/skipped
→ loại A/B. Ban đầu định theo Hướng C (`state.json` cạnh code), nhưng file state cục
bộ sẽ **mất** trên container ephemeral / VM mới → lần sau job re-embed nhầm toàn bộ.
Nên chốt **Hướng D**: bỏ hẳn `state.json`, lấy chính store làm nguồn sự thật.

Cách làm production trong code:
- **Hash nằm TRÊN document, không phải file ngoài** — [`upload_files`](../src/uploader.py)
  đóng dấu SHA-256 của nội dung vào `custom_metadata` mỗi document. Đầu mỗi run,
  [`delta.load_state_from_store`](../src/delta.py) list document trong store và dựng
  lại map `{slug: hash}` từ metadata đó → job **stateless**, chạy ở đâu cũng như nhau.
- **Upsert theo slug** — `upload_files` xóa document cũ cùng `display_name` trước khi
  index bản mới → update không đẻ duplicate, chạy lại job cũng không trùng (idempotent).
- **Store và state không thể lệch** — thứ đem đi diff chính là nội dung thật của store;
  xóa tay 1 document thì lần sau tự up lại (self-healing). Up thành công **chính là**
  cập nhật state (hash mới đóng dấu lên document) — không có bước ghi state riêng để lỡ.
- **`_wait` có timeout + check `operation.error`** — operation treo/lỗi thì fail lớn
  tiếng, không treo job hay báo nhầm thành công.
...
bài; muốn thêm thì diff slug hiện có với slug đang nằm trong store rồi xóa phần chênh.
